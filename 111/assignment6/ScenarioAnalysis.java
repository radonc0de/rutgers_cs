...
public class ScenarioAnalysis {
    ...
    public void computeCO2EmissionsAndCost () {
        ...
        for (int i = 0; i < vehicles.length; i++){
            ...
            //Calculates cost of Lease
            double leaseCost = vehicles[i].getLease().getDueAtSigning() + (vehicles[i].getLease().getNumberOfMonths() * vehicles[i].getLease().getMonthlyCost());

            if(vehicles[i].getFuel().getType() == 1){

                //Calculates Fuel Cost for Gas Cars
                double fuelCost = (vehicles[i].getLease().getNumberOfMonths() / 12) * (vehicles[i].getLease().getMileageAllowance()) * (1 / vehicles[i].getFuel().getUsage()) * getGasPrice();
                vehicles[i].setFuelCost(fuelCost);
                vehicles[i].setTotalCost(fuelCost + leaseCost + vehicles[i].getOtherCost());
            
                double emissions = (vehicles[i].getLease().getNumberOfMonths() / 12) * (vehicles[i].getLease().getMileageAllowance()) * (1 / vehicles[i].getFuel().getUsage()) * Fuel.CO2EMITTED_GASCOMBUSTION;
                ...
                //Calculates Fuel Cost for Electric Cars
                double fuelCost = (vehicles[i].getLease().getNumberOfMonths() / 12) * (vehicles[i].getLease().getMileageAllowance()) * (1 / vehicles[i].getFuel().getUsage()) * vehicles[i].getFuel().getKWhPerCharge() * getElectricityPrice() / 100;
                vehicles[i].setFuelCost(fuelCost);
                vehicles[i].setTotalCost(fuelCost + leaseCost + vehicles[i].getOtherCost());
                
                double emissions = (vehicles[i].getLease().getNumberOfMonths() / 12) * (vehicles[i].getLease().getMileageAllowance()) * (1 / vehicles[i].getFuel().getUsage()) * vehicles[i].getFuel().getKWhPerCharge() * Fuel.CO2EMITTED_GENERATEmWh * 0.00045;
                vehicles[i].setCO2Emission(emissions);
    ...
    public static void main (String[] args) {
        ScenarioAnalysis sa = new ScenarioAnalysis(3.84, 11.78);
