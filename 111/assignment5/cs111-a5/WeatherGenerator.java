...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        ...
        for(int i = 1; i < forecast.length; i++){
            ...
            if (forecast[i-1] == WET) {
                if(rand < wetwet[index][month]){
                    forecast[i] = WET;
                }else{
                    forecast[i] = DRY;
                }
            } else {
                if(rand < drywet[index][month]){
                    forecast[i] = WET;
                }else{
                    forecast[i] = DRY;
                }
    ...
    public static void main (String[] args) {
        ...
        //StdOut.println("There are " + numberOfWetDryDays(forecast, WET) + " wet days in " + month);
        //StdOut.println("There are " + numberOfWetDryDays(forecast, DRY) + " dry days in " + month);
